-- Función de prueba para verificar el setup de compañía
CREATE OR REPLACE FUNCTION test_company_setup()
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM public.profiles WHERE id = v_user_id) THEN
        RETURN json_build_object('success', false, 'error', 'El usuario ya tiene un perfil');
    END IF;
    
    -- Intentar crear una compañía de prueba
    BEGIN
        INSERT INTO public.companies (
            ...
            'Compañía de Prueba', 'Empresa de Prueba S.A.S', '900123456-1', 'test@empresa.com', '3001234567', 'Calle 123 #45-67',
            'Bogotá', 'Cundinamarca', '110111', 'Colombia', 'Simplificado',
            '6201', 'NIT', true
        );
        
        RETURN json_build_object('success', true, 'message', 'Compañía de prueba creada exitosamente');
...
-- Función mejorada para setup de compañía
CREATE OR REPLACE FUNCTION setup_company_and_profile_v2(
  p_user_id uuid,
  p_user_email text,
  p_user_name text,
  p_company_name text,
...
GRANT EXECUTE ON FUNCTION test_company_setup TO authenticated;

GRANT
EXECUTE ON FUNCTION setup_company_and_profile_v2 TO authenticated;