-- Migración completa para reconstruir el sistema de inventario
-- Esta migración corrige todos los problemas identificados

-- 1. Función para obtener inventario con filtros - COMPLETAMENTE REESCRITA
...
BEGIN
    ...
    v_query := format('
        ...
        LEFT JOIN public.warehouse_inventory wi ON p.id = wi.product_id 
            AND wi.warehouse_id = COALESCE(p.warehouse_id, (SELECT id FROM public.warehouses WHERE company_id = %L AND is_main = true LIMIT 1))
...
-- 2. Función para transferencias - COMPLETAMENTE REESCRITA
...
-- 3. Función para obtener transferencias - CORREGIDA
...
BEGIN
    ...
    -- Obtener transferencias con filtros usando subconsulta
    SELECT json_agg(
        json_build_object(
            'id', wt.id,
