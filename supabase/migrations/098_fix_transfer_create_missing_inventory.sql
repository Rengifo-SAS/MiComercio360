-- Migración para corregir transferencias cuando no existe inventario en bodega origen
-- Si no existe inventario en bodega origen, crear el registro con cantidad 0
-- Esto permite transferencias como si fuera un ingreso de producto a la bodega destino
...
-- Crear la función corregida que maneja inventario faltante
...
BEGIN
    ...
    -- Esto permite transferencias como si fuera un ingreso de producto
    IF v_existing_from_record IS NULL THEN
        ...
        v_from_quantity := 0;
        v_existing_from_record.quantity := 0;
    END IF;

    -- Verificar que hay suficiente stock en bodega origen
    -- Ahora permitimos transferencias aunque la cantidad sea 0 (como ingreso)
    ...
    RETURN json_build_object(
        ...
        'message', 'Transferencia completada. Producto ' || v_product_name || ' transferido desde ' || v_from_warehouse_name || ' hacia ' || v_to_warehouse_name
    );
