-- Corregir error SQL en función de búsqueda
-- 034_fix_search_function_sql_error.sql

-- Recrear la función search_products_advanced con SQL corregido
...
BEGIN
  ...
  -- Construir consulta principal con FROM correcto
  v_query := format('
...
-- También corregir la función get_search_stats
...
DECLARE
  ...
  v_query TEXT := '';
BEGIN
  -- Construir condiciones (mismo logic que search_products_advanced)
  IF p_search_term IS NOT NULL AND p_search_term != '' THEN
    v_search_condition := format(
      'AND (p.name ILIKE %L OR p.sku ILIKE %L OR p.description ILIKE %L)',
      '%' || p_search_term || '%',
...
COMMENT ON FUNCTION search_products_advanced IS 'Función corregida para búsqueda avanzada de productos con filtros múltiples';

COMMENT ON FUNCTION get_search_stats IS 'Función corregida para obtener estadísticas de búsqueda filtrada';