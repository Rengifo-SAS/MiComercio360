-- Migración definitiva para corregir el error de quantity null en transferencias
-- El problema está en el manejo de casos edge donde no existe inventario previo

-- Función completamente reescrita para transferencias con manejo robusto de NULL
...
BEGIN
    ...
    IF v_existing_from_record IS NOT NULL THEN
        ...
        END IF;
    ELSE
        -- No debería llegar aquí si hay stock insuficiente, pero por seguridad
        RETURN json_build_object('success', false, 'error', 'No existe inventario en bodega origen');
    END IF;
