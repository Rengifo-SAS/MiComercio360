...
interface SalesByItemReportClientProps {
  companyId: string;
  userId: string;
}
...
export function SalesByItemReportClient({
  companyId,
  userId,
}: SalesByItemReportClientProps) {
  const supabase = createClient();
  const [loading, setLoading] = useState(false);
  const [items, setItems] = useState<ItemSalesData[]>([]);
  const [filteredItems, setFilteredItems] = useState<ItemSalesData[]>([]);
  ...
  const generateReport = async () => {
    ...
    try {
      ...
      // Consultar ventas completadas y pendientes de pago
      ...
      const itemsMap = new Map<string, ItemSalesData>();
      ...
      salesData.forEach((sale: any) => {
        ...
        sale.sale_items.forEach((item: any) => {
          ...
          // Calcular monto antes de impuestos (subtotal del item)
          const unitPrice = Number(item.unit_price) || 0;
          const itemSubtotal = unitPrice * quantity;
          const itemDiscount = Number(item.discount_amount) || 0;
          const itemBeforeTax = itemSubtotal - itemDiscount;

          // Calcular monto después de impuestos (total_price ya incluye impuestos)
          const itemAfterTax = Number(item.total_price) || 0;

          itemData.amount_before_tax += itemBeforeTax;
          itemData.amount_after_tax += itemAfterTax;
        });
      });
      ...
      setCurrentPage(1); // Resetear a la primera página al generar nuevo reporte

      // Calcular totales
      const sumBeforeTax = itemsArray.reduce(
        (sum, item) => sum + item.amount_before_tax,
        0
      );
      const sumAfterTax = itemsArray.reduce(
        (sum, item) => sum + item.amount_after_tax,
        0
      );
  ...
  return (
    <div className="container mx-auto space-y-6 p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-center gap-2 text-sm text-muted-foreground">
        ...
        <span>/</span>
        <span className="text-foreground font-medium">Ventas por ítem</span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm md:text-base text-muted-foreground">
          Consulta la cantidad y monto vendido de cada ítem inventariable,
          incluyendo las ventas pendientes de pago.
        </p>
      </div>
      ...
      {items.length > 0 && (
        <Card className="shadow-sm">
          ...
          <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {paginatedItems.map((item, index) => (
                    <TableRow key={`${item.item_name}-${index}`} className="hover:bg-muted/50">
                      ...
                      <TableCell className="text-right">
                        <span className="text-primary font-medium">
                          {formatCurrency(item.amount_before_tax)}
                        </span>
                      </TableCell>
                      <TableCell className="text-right">
            ...
            {/* Información de paginación cuando solo hay una página o pocos items */}
            {filteredItems.length > 0 && totalPages === 1 && filteredItems.length > 0 && (
